-- Fix type casting issues in Best Deals functions
-- Cast varchar columns to text to match function return types

-------------------------------------------------------------------------------
-- Fix get_price_gap_deals with proper type casting
-------------------------------------------------------------------------------
...
BEGIN
  ...
  best_listings AS (
    SELECT DISTINCT ON (l.product_id)
      ...
      r.name::text AS best_retailer_name,
      l.image_url::text
  ...
  worst_listings AS (
    SELECT DISTINCT ON (l.product_id)
      ...
      r.name::text AS worst_retailer_name
    FROM product_price_ranges ppr
    JOIN listings l ON ppr.product_id = l.product_id
    JOIN retailers r ON l.retailer_id = r.id
...
-------------------------------------------------------------------------------
-- Fix get_price_history_deals with proper type casting
-------------------------------------------------------------------------------
